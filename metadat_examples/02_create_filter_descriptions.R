...
extract_metadata <- function(study){
  ...
  for (i in 1:rows){
    ...
    values <- try(paste0(as.character(c(unlist(unique(data[id]))),collapse='","')))
...
for (study in names(df_studies)){
  ...
  write_file(output,paste0('/Users/htr365/Documents/Side_Projects/09_founding_lab/semester_project/meta-studies/metadat_examples/filter_descriptions/',study,'.txt'))
